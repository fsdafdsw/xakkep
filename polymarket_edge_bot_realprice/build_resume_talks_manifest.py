...
def _question_meeting_subtype(row):
    return infer_meeting_subtype(row.get("question"), catalyst_type=row.get("catalyst_type"))
...
def _research_action(snapshot_row, repricing_row):
    ...
    if not repricing_row:
        ...
        if entry_price is not None and entry_price >= 0.80:
...
def _build_manifest_rows(manifest_rows, snapshot_index, repricing_index):
    output_rows = []
    for row in manifest_rows:
        if str(row.get("catalyst_type") or "") != "call_or_meeting":
            continue
        meeting_subtype = _question_meeting_subtype(row)
...
def _summary(rows, top_limit):
    ...
    return {
        ...
        "top_markets": rows[:top_limit],
    }


def parse_args():
    parser = argparse.ArgumentParser(description="Build a resume-talks research manifest from discovery, snapshot, and repricing outputs.")
